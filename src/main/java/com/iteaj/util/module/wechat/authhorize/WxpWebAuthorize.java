 ...
 * @since JDK1.7
 */
public class WxpWebAuthorize extends OAuth2ApiParam {

    private WechatScope scope; //授权域 有两种值：1.snsapi_userinfo 2.snsapi_base

    public WxpWebAuthorize(HttpServletRequest request, HttpServletResponse response) {
        this(request, response, WechatScope.UserInfo);
    }

    public WxpWebAuthorize(HttpServletRequest request
     ...
     * @param request
     * @param response
     * @param redirectUrl 此参数和{@link WxcWebAuthorize#redirectUrl}两者必填一处
     *                    , 如果两个地方都填,那么此处优先级更高
     */
    public WxpWebAuthorize(HttpServletRequest request
